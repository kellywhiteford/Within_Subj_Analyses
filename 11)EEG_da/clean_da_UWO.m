...
site = 'UWO';
%trigFile = {'RawData/Logs/da_klw_uwo_b1_triggers.mat','RawData/Logs/da_klw_uwo_b2_triggers.mat'}; % Name of .mat file with trigger polarity
...
for s = 1:size(subj,2) % for each subject
    ...
    if trig_16_inds ~= 6000
        error([fileName ': This bdf has ' num2str(trig_16_inds) ' triggers but there should be 6000 triggers.'])
    end
    
    triggers = zeros(1,6000); % Real triggers will be stored here.
    
    %% Load triggers from log file
    for b = 1:size(block,2) % for each block
        trigFile = strcat(folderName,'/',site,'/RawData/Logs/da_',subj{s},'_',block{b},'.mat'); % File name of raw data
    ...
    % Qualtiy-control check for UWO data
