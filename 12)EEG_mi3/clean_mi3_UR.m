...
for s = 1:size(subj,2) % for each subject
    for b = 1:size(block,2) % for each block
        ...
        fileName = strcat('mi3_',subj{s},'_',block{b},'.mat'); % File name of raw data
    ...
    % NOTE: For the within-subject pilot, the inter-stimulus-interval was 
    % 63 ms instead of 83 ms, so the timing between consecutive trigger 
    % onsets is only 278.5 + 63, rather than 278.5 + 83.
