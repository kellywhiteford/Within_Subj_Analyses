%% Script for formatting and error-checking UMN within-subjects pilot data
...
subj = {'klw_umn','klw2_umn'}; % subject and university ID -- Can include multiple subjects here
block = {'b1','b2'}; % block -- Each subject must have 2 blocks, labeled "b1" and "b2"
site = 'UMN';

%% Site-specific information
TriggerNums = [65409,65410]; % Site-specific trigger numbers 
OtherTrigs = 65408; % Other triggers indicating start/stop of blocks -- 65408 indicates the start of recording

% Fixed delay between the onset of the trigger and the arrival of the
% stimulus as the insert ear probe.
FixedDelay_ms = 1.93; % UMN: Includes ER1 earphone tube length plus digitial to analog conversion of sound wavefrom.
...
for s = 1:size(subj,2) % for each subject
    for b = 1:size(block,2) % for each block
        ...
        fileName = strcat('mi3_',subj{s},'_',block{b},'.bdf'); % File name of raw data
        ...
        % Quality-control check for UMN data
        ...
        elseif length(find(triggers==TriggerNums(1))) ~= 1200 || length(find(triggers==TriggerNums(2))) ~= 1200 % There should be 1200 trials per stimulus polarity.
