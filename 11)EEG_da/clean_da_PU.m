...
block = {'','+001','+002'}; % block -- PU system chunks data into bdf files < 1.05 GB 
site = 'PU';
...
FixedDelay_ms = 1.6; % PU: Includes ER1 earphone tube length plus digitial to analog conversion of sound wavefrom.
...
for s = 1:size(subj,2) % for each subject
    for b = 1:size(block,2) % for each block
        ...
        fileName = strcat('da_',subj{s},block{b},'.bdf'); % File name of raw data
